...
class Menu:
    ...
    def initialize_entry_line(self, text: str, x: int, y: int, width=200, height=50):
        ...
        text_box =  UITextBox(html_text=text, relative_rect=pygame.Rect((x, y), (width, height)), manager=self.manager)
        entry_line = UITextEntryLine(relative_rect=pygame.Rect( (x+250, y), (width/2, height)), manager=self.manager)
        return text_box, entry_line

    def initialize_selection(self, text: str, x: int, y: int, item_list:list, default:list, width=200, height=50):

        text_box = UITextBox(html_text=text, relative_rect=pygame.Rect((x, y), (width, height)), manager=self.manager)
        selection_list = UISelectionList(
        relative_rect=pygame.Rect((x+200, y), (width, y*2)),
        item_list=item_list,
    ...
    def initialize_button(self, text:str, x:int, y:int, width=100, height=50):
        return pygame_gui.elements.ui_button.UIButton(relative_rect=pygame.Rect((x, y), (width, height)),
                                                            text=text,
                                                     manager=self.manager)
